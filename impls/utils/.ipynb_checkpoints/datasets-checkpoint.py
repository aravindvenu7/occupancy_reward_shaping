...
@jax.jit
def compute_shaped_rewards(forward_model, inits, ends, rew_model = None, nexts = None):

    if rew_model is None:
        num_samples = 2
        batch_size, observation_dim = ends.shape[0], ends.shape[-1]
        ...
        x_1 = ends 
        vel = x_1 - x_0
        flow_steps = int(forward_model.config['flow_steps']*4.5)
        step_idx = jnp.arange(3, flow_steps, 3)
        t_vals = step_idx / flow_steps
        t = t_vals[:, None, None]
        x_t = (1.0 - t) * x_0[None, :, :] + t * x_1[None, :, :]

        '''
        rewards = []        
        # this loop is unnecessary and can be parallelized but keeping it here in case of low RAM GPUs. 
        ...
        rewards_time = jax.vmap(compute_reward_for_t, in_axes=(0, 0))(x_t, t_vals)
        rewards = jnp.mean(rewards_time, axis=0)
        assert rewards.shape[0] == batch_size, "Size mismatch in reward computation."
        rewards = jax.lax.stop_gradient(rewards)
        return -1.0 * rewards
    else:
        rewards = rew_model.compute_reward(inits, ends)
        return 0.75 * rewards
...
class GCDataset:
    ...
    def __post_init__(self):
        ...
        self.obs_mean = np.mean(self.dataset['observations'], axis = 0)
        self.obs_std = np.std(self.dataset['observations'], axis = 0)
        
        # Pre-compute trajectory boundaries. Standard code has bug, making modifications:
        (self.terminal_locs,) = np.nonzero(self.dataset['valids'] == 0) #np.nonzero(self.dataset['terminals'] > 0)
    ...
    def sample(self, batch_size, fwd_model = None, rew_model = None, idxs=None, evaluation=False,\
        ...
        initial_state_idxs = self.initial_locs[np.searchsorted(self.terminal_locs, idxs)]
        
        batch = self.dataset.sample(batch_size, idxs)
        batch['next_observations'] = self.get_observations(np.minimum(idxs + 1, final_state_idxs))  
        
        if self.config['append_fp_states']:
            batch['future_observations'] = self.get_future_observations(idxs)
            batch['next_actions'] = self.get_actions(np.minimum(idxs + 1, final_state_idxs))
        ...
        if self.config['reward_shaping']:
            ...
            if self.config['state_action']:
                ...
                inits = np.concatenate([inits, batch['actions']], axis = -1)
            else:
                inits = self.get_observations(np.minimum(idxs + 1, final_state_idxs))           

            batch['rewards'] = compute_shaped_rewards(fwd_model, inits, ends, rew_model = rew_model, \
                                                      nexts = batch['next_observations'])
            batch['rewards'] = jnp.where(successes, 1.0, batch['rewards'])
        else:
        ...
        if self.config['p_aug'] is not None and not evaluation:
            ...
            if np.random.rand() < self.config['p_aug']:
                if self.config['append_fp_states']:
                    self.augment(batch, ['observations', 'next_observations', 'future_observations', 'value_goals', 'actor_goals'])
                else:
    ...
    def get_future_observations(self, idxs):
        """Return the observations for the given indices."""
        if self.config['frame_stack'] is None or self.preprocess_frame_stack:
            future_obs_idx = jax.tree_util.tree_map(lambda arr: arr[idxs], self.dataset['future_obs_idx']) 
            future_obs = jax.tree_util.tree_map(lambda arr: arr[future_obs_idx], self.dataset['observations']) 
            return future_obs
        else:
            raise Exception("This is a forced exception. This part of the code has not been implemented.")
...
class HGCDataset(GCDataset):
    ...
    def sample(self, batch_size, fwd_model = None, rew_model = None, idxs=None, evaluation=False, \
        ...
        if self.config['reward_shaping'] and fwd_model is not None:
...
class GCOTADataset(GCDataset):
    ...
    def compute_value_next_idxs(self, idxs, final_state_idxs, high_goal_idxs, subgoal_steps):
        ...
        end_traj = subgoal_steps > final_state_idxs - idxs
        end_subgoal_steps = subgoal_steps.copy()
        ...
        return idxs + subgoal_steps, end_traj, subgoal_steps
    ...
    def sample(self, batch_size, fwd_model = None, rew_model = None, idxs=None, evaluation=False, \
        ...
        value_next_idxs, end_traj, effective_value_subgoal_steps = self.compute_value_next_idxs(
            idxs,
            final_state_idxs,
            value_goal_idxs,
            value_subgoal_steps,
        ...
        batch['value_goals'] = self.get_observations(value_goal_idxs)
        batch['value_subgoal_steps'] = effective_value_subgoal_steps
        batch['value_next_observations'] = self.get_observations(value_next_idxs)
        successes = (effective_value_subgoal_steps < value_subgoal_steps).astype(float) * (~end_traj)
        batch['masks'] = 1.0 - successes
        batch['actor_goals'] = self.get_observations(actor_goal_idxs)

        #batch['rewards'] = -1*(1 - self.config['discount'] ** effective_value_subgoal_steps)/(1 - self.config['discount'])
        batch['rewards'] = successes - (1.0 if self.config['gc_negative'] else 0.0)
