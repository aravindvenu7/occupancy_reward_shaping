...
class GCIQLOTAAgent(flax.struct.PyTreeNode):
    ...
    def compute_value(
        ...
        """
        (v1, v2) = self.network.select('value')(observation, goal)
        v = (v1 + v2) / 2
        return v
        
    def critic_loss(self, batch, grad_params):
        """Compute the IQL critic loss."""
        next_v = self.network.select('value')(batch['value_next_observations'], batch['value_goals'])

        q = batch['rewards'] + (self.config['discount']) * batch['masks'] * next_v
        #q = batch['rewards'] + (self.config['discount']**batch['value_subgoal_steps']) * batch['masks'] * next_v
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            agent_name='gciql_ota',  # Agent name.
            ...
            dataset_class='GCOTADataset',  # Dataset class name.
